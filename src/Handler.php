 ...
 * @license    https://github.com/aalfiann/reverser/blob/master/LICENSE.md  MIT License
 */
class Handler
{
    /**
     ...
     * @param bool $restapi If set to true then handler will act like rest json api server
     */
    function __construct($options,$restapi=false)
    {
        ...
        $this->_translatedUri = rtrim($options['uri'], '/');
        ...
        elseif (!empty($options['uri'])) {
            $baseUri = parse_url($options['uri'], PHP_URL_PATH);
    ...
    protected function readResponse(&$cu, $body)
    {
        ...
        // if they aren't passed from the handler application.
    ...
    /**
     * Executes the cURL handler, making the handler request.
    ...
    /**
     * Sets a new header that will be sent with the handler request
     *
     * @param string $headerName
     * @param string $value
     * @return void
